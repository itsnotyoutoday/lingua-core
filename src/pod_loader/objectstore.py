...
class ProfileNotConfigured(RuntimeError):
    """A named store was asked for and could not be resolved. Never silently substituted."""


#: Conventional key-file names per profile, searched upward to $HOME. Named for how they
#: actually appear on disk rather than for the profile, because that is what people write.
_PROFILE_FILES = {
    # RunPod's object endpoint is NOT S3 — no presigned URLs, batch delete 307s,
    # head_object 403s on large objects. Calling it "s3" in a variable name invited code
    # to assume capabilities it does not have, so it is named for the provider instead.
    "runpod": ("runpod-storage.key", "runpods3.key", "runpod-s3.key"),
    # Cloudflare IS S3-compatible, verified including presigned URLs. It is one store
    # among several that speak the protocol, so it is named for the vendor rather than
    # for the product — "R2" says nothing about what the code can do with it.
    "cloudflare": ("s3-cloudfare.key", "s3-cloudflare.key", "cloudflare.key"),
    "aws": ("aws-s3.key", "s3.key"),
    "minio": ("minio.key",),
}


def _profile_key_file(profile: str) -> str:
    from .storage import _search_up
    for name in _PROFILE_FILES.get(profile, (f"{profile}.key",)):
        p = _search_up(name)
        if p:
            return str(p)
    return ""
...
def resolve_config(profile: str | None = None) -> Any | None:
    ...
    # A NAMED profile that resolves to nothing must not fall through to the default key
    # file. That fallback meant get_storage("cloudflare") silently returned the RunPod store —
    # every profile answered with the same bucket, so "switch to Cloudflare" wrote to
    # RunPod and looked like it worked. A wrong store is worse than no store.
    if prof:
        kf = _env(prof, "KEY_FILE") or _profile_key_file(prof)
        if kf:
            cfg = load_config(kf)
            if cfg:
                return cfg
        raise ProfileNotConfigured(
            f"object-store profile {prof!r} is not configured.\n"
            f"  Set either:\n"
            f"      PODH_S3_{prof.upper()}_BUCKET / _ENDPOINT / _ACCESS / _SECRET\n"
            f"      PODH_S3_{prof.upper()}_KEY_FILE=/path/to/{prof}.key\n"
            f"  or place one of these beside the repo: "
            f"{', '.join(_PROFILE_FILES.get(prof, (prof + '.key',)))}\n"
            f"  Refusing to fall back to the default store: that silently sent writes "
            f"intended for {prof!r} somewhere else.")

    # The UNNAMED default may still come from the conventional key file, which remains the
    # right way to configure a laptop.
    return load_config()
